defmodule MiniShop.Catalog do
  ...
  alias MiniShop.Catalog.Product
  ...
  def subscribe_products(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(MiniShop.PubSub, "user:#{key}:products")
  end

  defp broadcast_product(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(MiniShop.PubSub, "user:#{key}:products", message)
  end
